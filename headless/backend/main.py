...
@app.post("/api/chatbots/{chatbot_id}/manager_send_message")
async def manager_send_message(
    chatbot_id: str,
    message: Message,
    user_id: str = Query(..., description="The ID of the user sending the message"),
    session_id: str = Query(..., description="The ID of the session")
):
    try:
        # Convert message Pydantic model to dictionary
        message_content = message.dict().get('message')
        ...
        await chatbots_collection.update_one(
            ...
            {'$push': {'sessions.$.messages': message_entry}}
        )
        ...
        try:
            ...
            await channel.publish('message', {"role": 'manager', "content": message_content})
...
async def send_message(
    ...
    try:
        ...
        if result.modified_count == 0:
            raise HTTPException(status_code=500, detail="Failed to update session with new message")
        ...
        if role == "user":
            ...
            if result.modified_count == 0:
                raise HTTPException(status_code=500, detail="Failed to update session with chatbot response")

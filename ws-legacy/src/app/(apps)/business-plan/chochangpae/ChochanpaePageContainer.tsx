...
export default function ChochangpaePageContainer({
  userId,
}: {
  userId: string;
}) {
  ...
  const handleAutoWriteAll = async () => {
    try {
      if (
        files.length === 0 &&
        prompt.type === "chochangpae" &&
        (prompt.chochangpae_a82443c5_794e_43e3_8dff_9a01d942e49c_item_name ===
          "" ||
          prompt.chochangpae_72857563_4ccf_4add_bf69_681a14a6bcfb_item_desc ===
            "")
      ) {
        setPopUpStatus({
          message:
  ...
  const handleConfirmRunAutoWriteAll = async () => {
    try {
      ...
      window.gtag("event", "conversion", {
        send_to: "AW-11469707361/3JcfCNW1i40ZEOGwl90q",
        event_callback: () => console.log("chochangpae callback"),
        currency: "KRW",
        value: 1.0,
      });
  ...
  return (
    <BusinessPlanFilesContext.Provider value={{ files, setFiles }}>
      <ChochangepaePromptContext.Provider value={{ prompt, setPrompt }}>
        <BusinessPlanTitle
          title="초기 창업 패키지 with docenty.ai"
          subTitle="창업사업화 지원사업 사업계획서 작성 AI 도우미, 도슨티 폼비서"
        />
        <Separator marginY="1rem" />
        <BusinessPlanInputForm type="chochangpae" />
        ...
        />
        {/* <BusinessPlanChatbot /> */}
      </ChochangepaePromptContext.Provider>
      ...
      <div className="flex flex-col rounded-[1rem] bg-[#FFFFFF] px-[3rem] py-[3rem] shadow-[0px_17px_17px_-7px_rgba(0,0,0,0.10)]">
        <div className="mb-[2rem] flex flex-col gap-[1.5rem]">
          <div className="text-[1.5rem] font-[700] text-[#0D0B33]">
            Free trial을 이용하셨습니다. 계속해서 서비스를 이용하려면, 멤버십
            업그레이드가 필요합니다
          </div>
          ...
          <div className="flex items-center gap-[0.5rem]">
            <div className="flex flex-col text-[0.875rem] font-[500] text-[#0D0B33]">
              <div>
                😄 설문조사를 해 주시면, 무료로 한달간 AI의 도움을 받을 수 있고,
              </div>
              <div>
                사업계획서 컨설팅, 평가, 첨삭 등 다양한 추가 기능을 먼저 만나실
                수 있습니다.
              </div>
            </div>
            <Link

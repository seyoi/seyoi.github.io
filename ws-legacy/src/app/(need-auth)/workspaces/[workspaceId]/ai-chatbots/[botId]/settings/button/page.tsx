...
import { PlugInSettingContextProvider } from "@/domains/ai-chatbot/stores/PlugInSettingContext";
import PlugInButtonImageForm from "@/domains/ai-chatbot/components/PlugInButtonImageForm";
import PlugInButtonPositionForm from "@/domains/ai-chatbot/components/PlugInButtonPositionForm";
import ExamplePlugInButton from "@/domains/ai-chatbot/components/ExamplePlugInButton";

export default async function AIChatbotSettingsButtonPage({
  params,
}: {
  params: { botId: string };
}) {
  const { botId } = params;
  const { plugIn, error } = await getBot({ botId });
  ...
  return (
    <div className="grid grid-rows-[90px,minmax(0,1fr)]">
      <div className="flex items-center justify-between border-b border-gray-lightest p-[20px]">
        <div className="flex items-center gap-[12px]">
          ...
          <div className="flex flex-col">
            <div className="text-lg">챗봇 설정 [버튼]</div>
      ...
      <div className="flex">
        <PlugInSettingContextProvider
          initialButtonImageUrl={plugIn?.buttonImageUrl || ""}
          initialButtonLabelText={plugIn?.buttonLabelText || ""}
          initialMarginX={plugIn?.marginX as number}
          initialMarginY={plugIn?.marginY as number}
        >
          <div className="flex flex-1 flex-col gap-[16px] bg-blue-lighter p-[20px]">
            <PlugInButtonImageForm />
            <PlugInButtonPositionForm />
          </div>

import { getWorkspace } from "@/domains/workspace/actions/getWorkspace";
import WorkspaceSettingForm from "@/domains/workspace/components/WorkspaceSettingForm";
...
export default async function WorkspaceSettingsPage({
  ...
  const { workspace: initialWorkspace, error } = await getWorkspace({
    workspaceId,
  });
    ...
    <div className="grid grid-rows-[90px,minmax(0,1fr)] overflow-x-auto">
      <div className="flex items-center gap-[12px] border-b border-gray-lightest p-[20px]">
        ...
        <div className="flex flex-col">
          <div className="text-lg">워크스페이스 설정</div>
          <div className="whitespace-nowrap text-sm text-gray-normal">
            워크스페이스의 정보를 수정해보세요.
          </div>
        </div>
      </div>
      <div className="flex h-full w-full bg-blue-lighter p-[20px]">
        <WorkspaceSettingForm initialWorkspace={initialWorkspace} />
      </div>
    </div>
  );
}

...
import { updateMyProfile } from "@/domains/workspace/actions/updateMyProfile";
...
export default function MyProfileForm() {
  ...
  const { myMembership } = useContext(WorkspaceContext);
  const [file, setFile] = useState<File | null>(null);
  const [avatarUrl, setAvatarUrl] = useState(myMembership?.avatarUrl as string);
  const [name, setName] = useState(myMembership?.name as string);
  const { addAlert } = useContext(AlertContext);

  const { channel: workspaceChannel } = useChannel(`workspace:${workspaceId}`);
  ...
  const handleClickSave = async () => {
    try {
      ...
      const { member, error } = await updateMyProfile({
        memberId: myMembership?.id as string,
        avatarUrl: uploadedAvatarUrl,
        name,
      });
      if (error) throw error;
      setAvatarUrl(member?.avatarUrl as string);
      setName(member?.name as string);
      addAlert({ type: "ok", text: "업데이트에 성공했습니다." });
      workspaceChannel.publish(`changeMe-${myMembership?.id}`, {
        myMembership: member,
      });
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="flex flex-col gap-[8px] rounded-[12px] border border-gray-lighter bg-white-normal p-[20px]">
      <div className="flex flex-col">
        <div className="text-lg">프로필 사진 변경</div>
      </div>
      <div className="flex flex-col gap-[16px]">
        ...
        <label htmlFor="email" className="flex flex-col gap-[8px]">
          <div id="email" className="text-md">
            이메일
          </div>
          <input
            className="rounded-[8px] border-[1.5px] border-gray-lighter bg-white-normal px-[12px] py-[8px] text-md placeholder:text-gray-light disabled:bg-gray-lightest"
            placeholder="Docenty Bot"
            value={myMembership?.email}
            disabled
          />
        </label>

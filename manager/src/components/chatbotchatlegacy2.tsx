  ...
  const sendMessage = async () => {
    if (input.trim() !== '') {
      ...
      try {
        ...
        console.log(result)
        // 매니저 챗이 받을 수 있도록 GPT-4의 답변을 해당 채널로 전송
        channel.publish('chatbot-message', { name: 'GPT-4', data: result.reply });

        setMessages(prevMessages => [
          ...prevMessages,
          { name: 'User', data: input },
          { name: 'GPT-4', data: result.reply }
        ]);
      } catch (error) {
        console.error('Error fetching GPT-4 reply:', error);
      }
